<template>
  <van-swipe-cell v-for="item in orderShippedList" :key="item.orderNumber">
    <van-card :num="item.number" :price="item.price" :title="item.detail">
      <template #thumb>
        <div class="g-thumb">
          <van-image :src="item.url" width="80%" @click="bindOrderDetail(item.orderNumber)"></van-image>
        </div>
      </template>
      <template #footer>
...
const { orderShippedList, bindDelete, bindOrderDetail } = useAllOrder()
</script>
